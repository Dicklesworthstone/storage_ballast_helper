...
| `/` | Log Search | Edit the search query inline (`Enter` runs it, `Esc` cancels); words match path/event/message, tokens `type:`, `level:`, `path:`, `id:`, `since:` |
...
Pools are planned per filesystem, not per mount entry. The mount holding a path is the one the kernel reports (`statfs`), so a macOS ballast dir under `/Users` lands on `/System/Volumes/Data` rather than the sealed, read-only `/`, and mount entries with the same source device (the bind mounts a `ProtectSystem=strict` unit sees for each `ReadWritePaths=` entry) fold into one pool on a writable entry, listed as "same filesystem as ..." and served by that pool. sbh ballast files left in a default `<mount>/.sbh/ballast` the daemon no longer manages (the pool moved to a configured `ballast_dir`, or its mount was folded) count as that filesystem's reserve and are released after the pool's own files; only files with the sbh ballast header are ever touched, and they are not replenished.
...
The emergency command scans the specified paths, scores candidates using the standard multi-factor engine, and presents them for immediate deletion. With `--yes`, it executes deletions immediately without interactive confirmation, prioritizing the highest-scoring candidates until the target free space is reached or all candidates are exhausted. Because no config is read, `--min-age <MINUTES>` (default 5) is the only age floor: artifacts modified more recently are left alone, and `--min-age 0` reclaims fresh builds too. Every candidate is printed with its decision id (`id <hex>` in the plan, `candidates[].decision_id` in `--json`), the same id `sbh scan` and `sbh clean` would record, so a later `sbh explain --id` can be matched against what emergency acted on.
...
The in-repo scanner-v2 validation tests emit focused JSON artifacts for synthetic v1/v2 walk effort and safety parity, event-overflow reconciliation fallback, and memory-pressure transition latency. v2 has been the default engine since v0.4.32 (it is what the production fleet runs and it is required for structural detection of caches under arbitrary names, such as Go's `GOCACHE`); the A/B evidence measured on 2026-09-02 is in `docs/internal/scanner-ab-2026-09-02.md` and does not yet meet the design document's promotion criteria (a one-shot `sbh scan` of a large project tree costs v2 3.4x the CPU of v1 because of its opaque-root size probes, and the daemon's v2 steady state was a replay hot loop rather than a cheap rescan), so `engine = "v1"` remains available as the rollback.

For a live A/B capture, one command runs v1 then v2 in-process over the same roots (the page cache is warmed by an untimed v2 pass first, so both measured passes see the same state) and emits the comparison artifact, including the promotion blocker `v2_candidates_vetoed_by_v1`:
...
Compare `scanned_entries`, `process_cpu_micros`, candidates, hard vetoes, and the `only_in_v1` / `only_in_v2` sets; v2 promotion requires `v2_candidates_vetoed_by_v1` to be empty.
...
- The daemon says where the bytes are: when a mount has stayed at Red or Critical for 10 minutes (reclaim is not keeping up), it logs an `SBH-2007` warning (journal and activity log, at most hourly per mount) naming the largest directories on that filesystem, with containers such as `/home` and `/data` broken down one level (`/home/ubuntu 278.1 GiB, /data/projects >=235.0 GiB, ...`; `>=` marks a size probe that hit its budget).
