...
impl ActivityLoggerHandle {
    ...
    /// Operational diagnostics intentionally coalesced, including lock
    /// contention. Separate from transport loss; shared by all handle clones.
...
fn logger_thread_main(
    ...
    // Summaries are generated by the consumer, never sent into its own queue.
    // A quiet period or disconnected producer must still flush pending counts.
    let mut last_reported_drops: u64 = 0;
    let mut last_drop_report = Instant::now();
    let mut last_diagnostic_check = Instant::now();
    ...
    loop {
        ...
        if d > 0
            && (stopping || now.saturating_duration_since(last_drop_report) >= diagnostics::WINDOW)
        {
            last_reported_drops = total_dropped;
            last_drop_report = now;
            let mut warn = LogEntry::new(EventType::Error, Severity::Warning);
            warn.error_code = Some("SBH-LOG-DROPPED".to_string());
            warn.details = Some(format!("{d} log events dropped due to back-pressure"));
            jsonl.write_entry(&warn);
        }
        let summary = if stopping
            || now.saturating_duration_since(last_diagnostic_check) >= diagnostics::WINDOW
        {
            last_diagnostic_check = now;
            diagnostics.take_report(now, stopping)
        } else {
            None
        };
        let event = if let Some(summary) = summary {
            summary
        } else if stopping {
            break;
        } else {
