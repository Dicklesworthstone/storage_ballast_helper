...
| Target | Tests |
| --- | --- |
| `cargo test --lib` (default features, with the TUI) | 2597 |
| `cargo test --lib --no-default-features --features cli,daemon,sqlite` (lean) | 1617 |
| `cargo test --bin sbh` | 145 |
| `cargo test --test cli_exit_codes` | 5 |
| `cargo test --test daemon_e2e` | 22 |
...
| Integration test files (sum of the `--test` rows) | 318 |
| E2E shell cases defined in `scripts/e2e_test.sh` | 129 |
| **Cargo tests on Linux, default features (lib + bin + integration)** | **3060** |
<!-- sbh-census:end -->
